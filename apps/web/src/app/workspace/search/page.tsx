        ...
        <article className="rounded-[1.75rem] border border-line bg-card p-6 shadow-[0_18px_45px_rgba(31,41,55,0.05)] md:p-8">
          ...
          <ul className="mt-6 space-y-3 text-sm leading-7 text-foreground">
            {plan.strategy.map((item) => (
              <li
                key={item}
                className="surface-muted rounded-[1.25rem] px-4 py-3"
              >
                {item}
              </li>
            ))}
    ...
    <main className="flex min-h-full flex-col gap-8">
      <section className="app-hero p-6 md:p-8">
        ...
        <div className="status-row mt-6">
          <div className="status-pill">Plan lisible avant execution</div>
          <div className="status-pill status-pill-info">Entreprises et pages carrieres</div>
          <div className="status-pill status-pill-success">Offres persistantes et detail ouvrable</div>
        </div>
      </section>
