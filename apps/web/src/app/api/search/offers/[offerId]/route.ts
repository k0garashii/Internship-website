...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  try {
    const result = await deletePersistedOfferForUser(viewer.userId, offerId);
    ...
    if (error instanceof SearchOfferListingError) {
      logRouteError({
        ...
        route: "/api/search/offers/[offerId]",
        request,
        message: "Offer deletion failed with a handled domain error",
        error,
        status: error.status,
        metadata: {
          userId: viewer.userId,
    ...
    logRouteError({
      route: "/api/search/offers/[offerId]",
      request,
      message: "Offer deletion failed with an unexpected error",
      error,
      status: 500,
      metadata: {
        userId: viewer.userId,
