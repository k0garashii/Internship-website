...
- `docs/architecture/gmail-mailbox-sync.md`: modele cible pour connecter Gmail, synchroniser la boite et suivre les reponses sans dependre des envois applicatifs
- `docs/architecture/gmail-permissions.md`: scopes Google/Gmail, revocation locale et routes de brouillon/envoi
...
- La strategie email est maintenant Gmail-first pour la lecture des reponses et candidatures, avec forwarding dedie conserve comme fallback MVP
- API `GET /api/email/google/connection` pour exposer l etat de la connexion Gmail et les messages synchronises
- API `GET /api/email/google/connect` et `GET /api/email/google/callback` pour connecter Gmail avec ajout progressif des scopes
- API `PATCH /api/email/google/settings` pour regler la requete de lecture Gmail
- API `POST /api/email/google/sync` pour synchroniser la boite et recalculer les reponses detectees
- API `POST /api/email/google/disconnect` pour debrancher Gmail cote application
...
- API `POST /api/email/drafts/[draftId]/gmail-draft` pour creer un brouillon Gmail
- API `POST /api/email/drafts/[draftId]/gmail-send` pour envoyer via Gmail
...
- Ecran `/workspace/email` pour connecter Gmail, regler la requete de synchronisation, visualiser les messages synchronises, suivre les reponses detectees et garder le forwarding comme fallback
- Ecran `/workspace/drafts` pour relire, modifier, sauvegarder, creer un brouillon Gmail ou envoyer directement les brouillons generes
